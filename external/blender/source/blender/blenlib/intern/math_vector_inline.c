...
MINLINE void zero_v2_d(double r[2])
{
	r[0] = 0.0;
	r[1] = 0.0;
}
...
MINLINE void copy_v2_v2_d(double r[2], const double a[2])
{
	r[0] = a[0];
	r[1] = a[1];
}
...
MINLINE void copy_v3_v3_d(double r[3], const double a[3])
{
	r[0] = a[0];
	r[1] = a[1];
	r[2] = a[2];
...
MINLINE void copy_v2_fl2_d(double v[2], double x, double y)
{
	v[0] = x;
	v[1] = y;
}
...
MINLINE void add_v2_v2_d(double r[2], const double a[2])
{
	r[0] += a[0];
	r[1] += a[1];
}
...
MINLINE void sub_v2_v2v2_d(double r[2], const double a[2], const double b[2])
{
	r[0] = a[0] - b[0];
	r[1] = a[1] - b[1];
}
...
MINLINE void mul_v2_fl_d(double r[2], double f)
{
	r[0] *= f;
	r[1] *= f;
}
...
MINLINE void mul_v2_v2fl_d(double r[2], const double a[2], double f)
{
	r[0] = a[0] * f;
	r[1] = a[1] * f;
}
...
MINLINE float dot_v2v2_d(const double a[2], const double b[2])
{
	return a[0] * b[0] + a[1] * b[1];
}
...
MINLINE float normalize_v2_v2_d(double r[2], const double a[2])
{
	float d = dot_v2v2_d(a, a);

	if (d > 1.0e-35f) {
		d = sqrt(d);
		mul_v2_v2fl_d(r, a, 1.0f / d);
	}
	else {
		zero_v2_d(r);
		d = 0.0f;
...
MINLINE float normalize_v2_d(double n[2])
{
	return normalize_v2_v2_d(n, n);
}
