...
typedef struct PVert {
	...
	union PVertUnion {
		...
		double distortion;       /* area smoothing */
...
static double p_vec_angle_cos(double *v1, double *v2, double *v3)
{
	double d1[3], d2[3];
...
static double p_vec_angle(double *v1, double *v2, double *v3)
{
...
static double p_vec2_angle(double *v1, double *v2, double *v3)
{
	double u1[3], u2[3], u3[3];

	u1[0] = v1[0]; u1[1] = v1[1]; u1[2] = 0.0;
	u2[0] = v2[0]; u2[1] = v2[1]; u2[2] = 0.0;
	u3[0] = v3[0]; u3[1] = v3[1]; u3[2] = 0.0;

	return p_vec_angle(u1, u2, u3);
}

static void p_triangle_angles(double *v1, double *v2, double *v3, double *a1, double *a2, double *a3)
{
...
static void p_face_angles(PFace *f, double *a1, double *a2, double *a3)
{
...
static double p_face_uv_area_signed(PFace *f)
{
	PEdge *e1 = f->edge, *e2 = e1->next, *e3 = e2->next;
	PVert *v1 = e1->vert, *v2 = e2->vert, *v3 = e3->vert;
...
static double p_edge_uv_length(PEdge *e)
{
	PVert *v1 = e->vert, *v2 = e->next->vert;
	double d[3];
...
static void p_chart_uv_bbox(PChart *chart, double minv[2], double maxv[2])
{
	PVert *v;
...
static void p_chart_uv_scale(PChart *chart, double scale)
{
	PVert *v;
...
static void p_chart_uv_scale_xy(PChart *chart, double x, double y)
{
	PVert *v;
...
static void p_chart_uv_translate(PChart *chart, double trans[2])
{
	PVert *v;
...
static void p_chart_uv_transform(PChart *chart, double mat[2][2])
{
	PVert *v;
...
static void p_chart_uv_to_array(PChart *chart, double(*points)[2])
{
	PVert *v;
	unsigned int i = 0;
...
static void UNUSED_FUNCTION(p_chart_uv_from_array)(PChart *chart, double(*points)[2])
{
	PVert *v;
	unsigned int i = 0;
...
static PBool p_intersect_line_2d_dir(double *v1, double *dir1, double *v2, double *dir2, double *isect)
{
	double lmbda, div;
...
#if 0
static PBool p_intersect_line_2d(double *v1, double *v2, double *v3, double *v4, double *isect)
{
	...
	if (!p_intersect_line_2d_dir(v1, dir1, v2, dir2, isect)) {
		...
		* let's give a point nearby in case things go wrong */
		isect[0] = (v1[0] + v2[0]) * 0.5;
		isect[1] = (v1[1] + v2[1]) * 0.5;
		return P_FALSE;
	}
...
static void p_face_flip(PFace *f)
{
	...
	double *orig_uv1 = e1->orig_uv, *orig_uv2 = e2->orig_uv, *orig_uv3 = e3->orig_uv;
...
static void p_flush_uvs_blend(PHandle *handle, PChart *chart, double blend)
{
	PEdge *e;
...
static PVert *p_vert_add(PHandle *handle, PHashKey key, const double co[3], PEdge *e)
{
	PVert *v = (PVert *)BLI_memarena_alloc(handle->arena, sizeof(*v));
	copy_v3_v3_d(v->co, co);
	v->u.key = key;
	v->edge = e;
	v->flag = 0;
...
static PVert *p_vert_lookup(PHandle *handle, PHashKey key, const double co[3], PEdge *e)
{
...
static PFace *p_face_add_construct(PHandle *handle, ParamKey key, ParamKey *vkeys,
	double *co[4], double *uv[4], int i1, int i2, int i3,
...
static PBool p_quad_split_direction(PHandle *handle, double **co, PHashKey *vkeys)
{
	...
	* depending on floating point errors to decide */
	double bias = 1.0 + 1e-6;
	double fac = len_v3v3(co[0], co[2]) * bias - len_v3v3(co[1], co[3]);
	PBool dir = (fac <= 0.0);
...
static double p_edge_boundary_angle(PEdge *e)
{
	PEdge *we;
	PVert *v, *v1, *v2;
	double angle;
	...
	if (nedges == 2) {
		...
		while (nedges > 2) {
			...
			if (nedges == 3) {
				...
				ne2->u.heaplink = BLI_heap_insert(heap, (float)p_edge_boundary_angle(ne2), ne2);
				e2->u.heaplink = BLI_heap_insert(heap, (float)p_edge_boundary_angle(e2), e2);
			}
...
static int p_polygon_point_in(double *cp1, double *cp2, double *p)
{
	if ((cp1[0] == p[0]) && (cp1[1] == p[1]))
		...
		return (p_area_signed(cp1, cp2, p) >= 0.0);
}

static void p_polygon_kernel_clip(double(*oldpoints)[2], int noldpoints, double(*newpoints)[2], int *nnewpoints, double *cp1, double *cp2)
{
	double *p2, *p1, isect[2];
	int i, p2in, p1in;
...
static void p_polygon_kernel_center(double(*points)[2], int npoints, double *center)
{
	int i, size, nnewpoints = npoints;
	double(*oldpoints)[2], (*newpoints)[2], *p1, *p2;

	size = npoints * 3;
	oldpoints = MEM_mallocN(sizeof(double) * 2 * size, "PPolygonOldPoints");
	newpoints = MEM_mallocN(sizeof(double) * 2 * size, "PPolygonNewPoints");

	memcpy(oldpoints, points, sizeof(double) * 2 * npoints);
	...
	for (i = 0; i < npoints; i++) {
		...
		if (nnewpoints * 2 > size) {
			size *= 2;
			MEM_freeN(oldpoints);
			oldpoints = MEM_mallocN(sizeof(double) * 2 * size, "oldpoints");
			memcpy(oldpoints, newpoints, sizeof(double) * 2 * nnewpoints);
			MEM_freeN(newpoints);
			newpoints = MEM_mallocN(sizeof(double) * 2 * size, "newpoints");
		}
		else {
			double(*sw_points)[2] = oldpoints;
			oldpoints = newpoints;
			newpoints = sw_points;
		}
	}
...
static double p_vert_cotan(double *v1, double *v2, double *v3)
{
	double a[3], b[3], c[3], clen;
...
static PBool p_vert_flipped_wheel_triangle(PVert *v)
{
	...
	do {
		if (p_face_uv_area_signed(e->face) < 0.0)
			return P_TRUE;
...
{
	double weightsum, positionsum[2], olduv[2];

	weightsum = 0.0;
	positionsum[0] = positionsum[1] = 0.0;
...
static void p_vert_harmonic_insert(PVert *v)
{
	...
	if (!p_vert_map_harmonic_weights(v)) {
		...
		* weights fails */
		...
		points = MEM_mallocN(sizeof(double) * 2 * npoints, "PHarmonicPoints");
...
static PBool p_collapse_normal_flipped(double *v1, double *v2, double *vold, double *vnew)
{
	double nold[3], nnew[3], sub1[3], sub2[3];
...
static PBool p_collapse_allowed_geometric(PEdge *edge, PEdge *pair)
{
	...
	e = oldv->edge;
	do {
		double a[3], b[3], minangle, maxangle;
...
static double p_collapse_cost(PEdge *edge, PEdge *pair)
{
	...
	* "Fast and Memory Efficient Polygonal Simplification" P. Lindstrom, G. Turk */
	...
	PEdge *e;
	PFace *oldf1, *oldf2;
	double volumecost = 0.0, areacost = 0.0, edgevec[3], cost, weight, elen;
	double shapecost = 0.0;
	double shapeold = 0.0, shapenew = 0.0;
	...
	do {
		double a1, a2, a3;
		double *co1 = e->next->vert->co;
		double *co2 = e->next->next->vert->co;

		if ((e->face != oldf1) && (e->face != oldf2)) {
			double tetrav2[3], tetrav3[3], c[3];
			...
			volumecost += fabs(dot_v3v3(edgevec, c) / 6.0);
#if 0
...
static void p_collapse_cost_vertex(PVert *vert, double *mincost, PEdge **mine)
{
	PEdge *e, *enext, *pair;
	...
	*mincost = 0.0;
	...
	do {
		if (p_collapse_allowed(e, e->pair)) {
			double cost = p_collapse_cost(e, e->pair);
		...
		if (enext == NULL) {
			...
			if (p_collapse_allowed(NULL, pair)) {
				double cost = p_collapse_cost(NULL, pair);
...
static void p_abf_setup_system(PAbfSystem *sys)
{
	...
	sys->alpha = (double *)MEM_mallocN(sizeof(double) * sys->nangles, "ABFalpha");
	sys->beta = (double *)MEM_mallocN(sizeof(double) * sys->nangles, "ABFbeta");
	sys->sine = (double *)MEM_mallocN(sizeof(double) * sys->nangles, "ABFsine");
	...
	sys->lambdaTriangle = (double *)MEM_callocN(sizeof(double) * sys->nfaces, "ABFlambdatri");
...
{
	int i;
	double *sine = sys->sine, *cosine = sys->cosine, *alpha = sys->alpha;
...
static double p_abf_compute_sin_product(PAbfSystem *sys, PVert *v, int aid)
{
	PEdge *e, *e1, *e2;
	double sin1, sin2;
...
static double p_abf_compute_grad_alpha(PAbfSystem *sys, PChart *chart, PFace *f, PEdge *e)
{
	...
	if (v1->flag & PVERT_INTERIOR) {
		double product = p_abf_compute_sin_product(sys, v1, e->u.id);
...
static double p_abf_compute_gradient(PAbfSystem *sys, PChart *chart)
{
	PFace *f;
	PEdge *e;
	PVert *v;
...
static PBool p_abf_matrix_invert(PAbfSystem *sys, PChart *chart, double norm_gradient, bool *noFurtherIterations)
{
	...
	if (success) {
		for (f = chart->faces; f; f = f->nextlink) {
			...
			if (v1->flag & PVERT_INTERIOR) {
				double x = EIG_linear_solver_variable_get(context, 0, v1->u.id);
				double x2 = EIG_linear_solver_variable_get(context, 0, ninterior + v1->u.id);
			...
			if (v2->flag & PVERT_INTERIOR) {
				double x = EIG_linear_solver_variable_get(context, 0, v2->u.id);
				double x2 = EIG_linear_solver_variable_get(context, 0, ninterior + v2->u.id);
			...
			if (v3->flag & PVERT_INTERIOR) {
				double x = EIG_linear_solver_variable_get(context, 0, v3->u.id);
				double x2 = EIG_linear_solver_variable_get(context, 0, ninterior + v3->u.id);
...
static void p_chart_abf_init(PChart *chart) {
	PFace *f;
	PVert *v;

	for (f = chart->faces; f; f = f->nextlink) {
		f->flag &= ~PFACE_ABF_SET;
	}

	for (v = chart->verts; v; v = v->nextlink) {
		v->flag &= ~PVERT_ABF_SET;
	}
}

static inline void p_chart_abf_calc_missing_vert_of_triangle(double *alphas, PEdge *e1, bool first) {
	PEdge *e2, *e3;

	e2 = e1->next;
	e3 = e2->next;

	double c = p_edge_uv_length(e1);
	...
	double alpha = alphas[e1->u.id];
	double beta = alphas[e2->u.id];
	double gamma = alphas[e3->u.id];
	...
	if (sinGamma > -1e-8 && sinGamma < 1e-8) {
		...
		sub_v2_v2v2_d(xAxis, e2->vert->uv, e1->vert->uv);
		normalize_v2_d(xAxis);

		copy_v2_fl2_d(yAxis, -xAxis[1], xAxis[0]);
		...
		add_v2_v2_d(e3->vert->uv, e1->vert->uv);
		add_v2_v2_d(e3->vert->uv, xAxis);
		add_v2_v2_d(e3->vert->uv, yAxis);
	}
}

static inline void p_chart_abf_set_vertex(double *alpha, struct BLI_Stack *edgeStack, PEdge *edge) {
	PEdge *e2, *e3;
	PFace *face = edge->face;

	if (face->flag & PFACE_ABF_SET) {
		return;
	}

	face->flag |= PFACE_ABF_SET;
	...
	copy_v2_fl2_d(e3->vert->uv, 0, 0);

	p_chart_abf_calc_missing_vert_of_triangle(alpha, edge, true);
	p_chart_abf_calc_missing_vert_of_triangle(alpha, edge, false);
...
static PBool p_chart_symmetry_pins(PChart *chart, PEdge *outer, PVert **pin1, PVert **pin2)
{
	...
	double maxlen = 0.0, curlen = 0.0, totlen = 0.0, firstlen = 0.0;
	double len1, len2;
	...
	* marked during construction */
	...
	if (!maxe1 || !maxe2 || (maxlen < 0.5 * totlen))
		return P_FALSE;
...
static PBool p_chart_lscm_solve(PHandle *handle, PChart *chart)
{
	...
	for (f = chart->faces; f; f = f->nextlink) {
		...
		if ((v1->flag & PVERT_PIN) && (v2->flag & PVERT_PIN) && (v3->flag & PVERT_PIN)) {
			double area = p_face_uv_area_signed(f);
	...
	for (f = chart->faces; f; f = f->nextlink) {
		PEdge *e1 = f->edge, *e2 = e1->next, *e3 = e2->next;
		PVert *v1 = e1->vert, *v2 = e2->vert, *v3 = e3->vert;
		double a1, a2, a3, ratio, cosine, sine;
		double sina1, sina2, sina3, sinmax;
		...
		ratio = (sina3 == 0.0) ? 1.0 : sina2 / sina3;
		cosine = cos(a1) * ratio;
		sine = sina1 * ratio;

		EIG_linear_solver_matrix_add(context, row, 2 * v1->u.id, cosine - 1.0);
		...
		EIG_linear_solver_matrix_add(context, row, 2 * v1->u.id + 1, cosine - 1.0);
...
static double p_face_stretch(PFace *f)
{
	double T, w, tmp[3];
	double Ps[3], Pt[3];
	double a, c, area;
	...
	if (area <= 0.0) /* flipped face -> infinite stretch */
		return 1e10;
...
static double p_stretch_compute_vertex(PVert *v)
{
	PEdge *e = v->edge;
	double sum = 0.0;
...
static void p_chart_stretch_minimize(PChart *chart, RNG *rng)
{
	...
	PEdge *e;
	int j, nedges;
	double orig_stretch, low, stretch_low, high, stretch_high, mid, stretch;
	double orig_uv[2], dir[2], random_angle, trusted_radius;
	...
	for (v = chart->verts; v; v = v->nextlink) {
		...
		random_angle = BLI_rng_get_double(rng) * 2.0 * M_PI;
...
static double p_rectangle_area(double *p1, double *dir, double *p2, double *p3, double *p4)
{
	/* given 4 points on the rectangle edges and the direction of on edge,
	* compute the area of the rectangle */

	double orthodir[2], corner1[2], corner2[2], corner3[2];
...
static double p_chart_minimum_area_angle(PChart *chart)
{
	/* minimum area enclosing rectangle with rotating calipers, info:
	* http://cgm.cs.mcgill.ca/~orm/maer.html */

	double rotated, minarea, minangle, area, len;
	double *angles, miny, maxy, v[2], a[4], mina;
	...
	angles = MEM_mallocN(sizeof(double) * npoints, "PMinAreaAngles");
...
{
	double angle = p_chart_minimum_area_angle(chart);
	double sine = sin(angle);
	double cosine = cos(angle);
	PVert *v;

	for (v = chart->verts; v; v = v->nextlink) {
		double oldu = v->uv[0], oldv = v->uv[1];
...
typedef struct SmoothTriangle {
	double co1[2], co2[2], co3[2];
	double oco1[2], oco2[2], oco3[2];
} SmoothTriangle;
...
static void p_barycentric_2d(const double v1[2], const double v2[2], const double v3[2], const double p[2], double b[3])
{
	double a[2], c[2], h[2], div;
...
static PBool p_triangle_inside(SmoothTriangle *t, double co[2])
{
	double b[3];
...
static SmoothNode *p_node_new(MemArena *arena, SmoothTriangle **tri, int ntri, double *bmin, double *bmax, int depth)
{
	SmoothNode *node = BLI_memarena_alloc(arena, sizeof(*node));
	int axis, i, t1size = 0, t2size = 0;
	double split, /* mi, */ /* UNUSED */ mx;
...
static PBool p_node_intersect(SmoothNode *node, double co[2])
{
	int i;
...
static int p_compare_double(const void *a_, const void *b_)
{
...
static double p_smooth_median_edge_length(PChart *chart)
{
	PEdge *e;
	double *lengths = MEM_mallocN(sizeof(chart->edges) * chart->nedges, "PMedianLength");
	double median;
	int i;
	...
	qsort(lengths, i, sizeof(double), p_compare_double);
...
static double p_smooth_distortion(PEdge *e, double avg2d, double avg3d)
{
	double len2d = p_edge_uv_length(e) * avg3d;
	double len3d = p_edge_length(e) * avg2d;

	return (len3d == 0.0) ? 0.0 : len2d / len3d;
}
...
static void p_smooth(PChart *chart)
{
	...
	double minv[2], maxv[2], median, invmedian, avglen2d, avglen3d;
	double center[2], dx, dy, *nodes, dlimit, d, *oldnodesx, *oldnodesy;
	double *nodesx, *nodesy, *hedges, *vedges, climit, moved, padding;
	...
	median = p_smooth_median_edge_length(chart) * 0.1;
	...
	center[0] = 0.5 * (minv[0] + maxv[0]);
	center[1] = 0.5 * (minv[1] + maxv[1]);

	dx = 0.5 * (maxv[0] - minv[0]);
	dy = 0.5 * (maxv[1] - minv[1]);

	padding = 0.15;
	dx += padding * dx + 2.0 * median;
	dy += padding * dy + 2.0 * median;
	...
	nodes = MEM_mallocN(sizeof(double) * nsize, "PSmoothNodes");
	nodesx = MEM_mallocN(sizeof(double) * nsize, "PSmoothNodesX");
	nodesy = MEM_mallocN(sizeof(double) * nsize, "PSmoothNodesY");
	oldnodesx = MEM_mallocN(sizeof(double) * nsize, "PSmoothOldNodesX");
	oldnodesy = MEM_mallocN(sizeof(double) * nsize, "PSmoothOldNodesY");
	hedges = MEM_mallocN(sizeof(double) * esize, "PSmoothHEdges");
	vedges = MEM_mallocN(sizeof(double) * esize, "PSmoothVEdges");
	...
	for (it = 0; it < maxiter; it++) {
		...
		for (x = 0; x < edgesx; x++) {
			for (y = 0; y < edgesy; y++) {
				...
				hedges[j] = (nodes[i] + nodes[i + 1]) * 0.5;
				vedges[j] = (nodes[i] + nodes[i + gridx]) * 0.5;

				/* we do *inverse* mapping */
				hedges[j] = 1.0 / hedges[j];
				vedges[j] = 1.0 / vedges[j];
			}
		}
		...
		for (it2 = 0; it2 < maxiter2; it2++) {
			...
			memcpy(oldnodesx, nodesx, sizeof(double) * nsize);
			memcpy(oldnodesy, nodesy, sizeof(double) * nsize);

			for (x = 1; x < gridx - 1; x++) {
				for (y = 1; y < gridy - 1; y++) {
					double p[2], oldp[2], sum1, sum2, diff[2], length;
...
void param_aspect_ratio(ParamHandle *handle, double aspx, double aspy)
{
...
static void p_add_ngon(ParamHandle *handle, ParamKey key, int nverts,
	ParamKey *vkeys, double **co, double **uv,
	ParamBool *pin, ParamBool *select, const double normal[3])
{
	...
	while (nverts > 2) {
		double minangle = FLT_MAX;
		double minshape = FLT_MAX;
		int i, mini = 0;
		...
		for (i = 0; i < nverts; i++) {
			...
			double angle = p_vec_angle(co[v0], co[v1], co[v2]);
			double n[3];

			normal_tri_v3(n, co[v0], co[v1], co[v2]);

			if (normal && (dot_v3v3(n, normal) < 0.0))
				angle = (2.0 * M_PI) - angle;

			double other_angle = p_vec_angle(co[v2], co[v0], co[v1]);
			double shape = fabs(M_PI - angle - 2.0 * other_angle);

			if (fabs(angle - minangle) < 0.01) {
		...
		/* add triangle in corner */
		{
			...
			double *tri_co[3] = { co[v0], co[v1], co[v2] };
			double *tri_uv[3] = { uv[v0], uv[v1], uv[v2] };
...
void param_face_add(ParamHandle *handle, ParamKey key, int nverts,
	ParamKey *vkeys, double *co[4], double *uv[4],
	ParamBool *pin, ParamBool *select, double normal[3])
{
	PHandle *phandle = (PHandle *)handle;
...
void param_abf_begin(ParamHandle *handle)
{
	...
	phandle->state = PHANDLE_STATE_ABF;
...
void param_abf_end(ParamHandle *handle)
{
	...
	for (i = 0; i < phandle->ncharts; i++) {
		p_chart_abf_end(phandle->charts[i]);
	}
...
void param_stretch_blend(ParamHandle *handle, double blend)
{
	PHandle *phandle = (PHandle *)handle;
...
static void param_pack_rotate(ParamHandle *handle)
{
	...
	for (i = 0; i < phandle->ncharts; i++) {
		...
		angle = BLI_convexhull_aabb_fit_points_2d((const double(*)[2])points, chart->nverts);
...
void param_pack(ParamHandle *handle, double margin, bool do_rotate)
{
	/* box packing variables */
	BoxPack *boxarray, *box;
	double tot_width, tot_height, scale;
	...
	if (phandle->aspx != phandle->aspy)
		param_scale(handle, 1.0 / phandle->aspx, 1.0 / phandle->aspy);
...
void param_average(ParamHandle *handle)
{
	PChart *chart;
	int i;
	double tot_uvarea = 0.0, tot_facearea = 0.0;
	double tot_fac, fac;
	double minv[2], maxv[2], trans[2];
	PHandle *phandle = (PHandle *)handle;
	...
	for (i = 0; i < phandle->ncharts; i++) {
		...
		chart->u.pack.area = 0.0; /* 3d area */
		chart->u.pack.rescale = 0.0; /* UV area, abusing rescale for tmp storage, oh well :/ */
	...
	for (i = 0; i < phandle->ncharts; i++) {
		...
		if (chart->u.pack.area != 0.0 && chart->u.pack.rescale != 0.0) {
			fac = chart->u.pack.area / chart->u.pack.rescale;

			/* Get the island center */
			p_chart_uv_bbox(chart, minv, maxv);
			trans[0] = (minv[0] + maxv[0]) / -2.0;
			trans[1] = (minv[1] + maxv[1]) / -2.0;
...
void param_scale(ParamHandle *handle, double x, double y)
{
	PHandle *phandle = (PHandle *)handle;
	PChart *chart;
	int i;
